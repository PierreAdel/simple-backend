...
async function insertOne(collectionName, insert, options = {}) {
  const insertBody = {
    ...insert,
    ...(!insert.createdAt && { createdAt: new Date() }),
  };
  const acknowledgement = await mongoModule.db
    .collection(collectionName)
    .insertOne(insertBody, options);
...
async function aggregate(collectionName, pipeline, options) {
  const documents = await mongoModule.db
    .collection(collectionName)
    .aggregate(
      Object.values(_.pick(pipeline, Object.keys(_.omit(pipeline, ["count"])))),
      options,
    )
    .toArray();
  let count = 0;
...
async function aggregateById(collectionName, pipeline, options) {
  const documents = await mongoModule.db
    .collection(collectionName)
    .aggregate(
      Object.values(Object.values(_.pick(pipeline, Object.keys(pipeline)))),
      options,
    )
    .toArray();
...
async function findOneAndUpdateById(
  collectionName,
  conditions,
  update,
  options,
) {
  const document = await mongoModule.db
    .collection(collectionName)
...
async function deleteOneById(collectionName, conditions, options) {
  const ack = await mongoModule.db
    .collection(collectionName)
