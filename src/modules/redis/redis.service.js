...
async function set(key, result, shouldExpire = false, seconds = 60 * 60) {
  let ack;
  if (shouldExpire)
    ack = await client.set(key, JSON.stringify(result), "EX", seconds);
...
async function updateByIdInvalidation(collectionName, idValue, updateKeys) {
  ...
  const hashesKeys = results.map((result) =>
  ...
  hashesKeys.forEach((hashKeys, i) =>
    hashKeys.forEach((key) => {
      ...
      if (optionsOrPipeline.projection)
        projectionObject = optionsOrPipeline.projection;
      if (optionsOrPipeline.project?.$project)
        projectionObject = optionsOrPipeline.project?.$project;
      const projectionKeys = Object.keys(projectionObject);
      if (projectionKeys.length) {
        if (projectionKeys.filter((prop) => updateKeys.includes(prop)).length)
          keysToBeDeleted[i].push(key);
      } else keysToBeDeleted[i].push(key);
    }),
  );
  const pipeline2 = client.pipeline();
  if (keysToBeDeleted[0].length)
    pipeline2.hdel(`${collectionName}`, ...keysToBeDeleted[0]);
  if (keysToBeDeleted[1].length)
    pipeline2.hdel(`${collectionName}:${idValue}`, ...keysToBeDeleted[1]);
