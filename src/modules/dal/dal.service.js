...
async function readOne(collectionName, filter, options, idValue) {
  options.projection = dalUtil.projectionObject(
    collectionName,
    options.projection,
  );
  const cached = await redisService.hget(`${collectionName}:${idValue}`, {
    ...filter,
    ...options,
  });
...
async function readMany(collectionName, filter, options) {
  ...
  const cached = await redisService.hget(collectionName, {
    ...filter,
    ...options,
  });
  if (cached) return cached;
...
async function aggregateReadOne(collectionName, pipeline, options, idValue) {
  pipeline.project = dalUtil.aggregateProjectionObject(
    collectionName,
    pipeline,
  );
  const cached = await redisService.hget(`${collectionName}:${idValue}`, {
  ...
  if (cached) return cached;
  const document = await mongoService.aggregateById(
    collectionName,
    pipeline,
    options,
  );
...
async function updateOne(
  ...
  const document = await mongoService.findOneAndUpdateById(
    collectionName,
    conditions,
    update,
    options,
  ...
  if (document)
    await redisService.updateByIdInvalidation(
      collectionName,
      idValue,
      updateKeys,
    );
...
async function updateMany(collectionName, conditions, update, options) {
  const ack = await mongoService.updateMany(
    collectionName,
    conditions,
    update,
    options,
...
async function deleteOne(collectionName, conditions, options, idValue) {
  const deleted = await mongoService.deleteOneById(
    collectionName,
    conditions,
    options,
  );
...
async function deleteMany(collectionName, conditions, options) {
  const ack = await mongoService.deleteMany(
    collectionName,
    conditions,
    options,
  );
